...
def send_point_usage_to_firestore(points,description) :
    ...
    user_ref.collection('point').add(point_usage_data)
...
def send_generate_result_to_firestore(service_name, points, result):
    ...
    send_point_usage_to_firestore(points, service_name)

def send_stats_to_firestore(service_name):
    today = date.today()
    user_ref = db.collection('stats')
    generate_result_data = {
        "time": datetime.now(timezone.utc),
        "service_name" : service_name
    }
    user_ref.add(generate_result_data)